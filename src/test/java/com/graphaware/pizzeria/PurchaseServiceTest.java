...
public class PurchaseServiceTest {
	...
	@Test
	void should_have_pinapple_discount() {
		Pizza one = new Pizza();
		one.setId(666L);
		one.setToppings(Collections.singletonList("pineapple"));
		one.setPrice(12.0);
		Pizza two = new Pizza();
		two.setId(43L);
		two.setPrice(10.0);
		...
		toReturn.setState(PurchaseState.ONGOING);
		when(purchaseRepository.findById(any())).thenReturn(Optional.of(toReturn));

		purchaseService.confirmPurchase();

		purchaseService.pickPurchase();

		purchaseService.completePurchase(toReturn.getId());

		assertEquals(toReturn.getAmount(), 21.0);
	}

	@Test
	void should_have_three_pizza_discount() {
		Pizza one = new Pizza();
		one.setId(666L);
		one.setToppings(Collections.singletonList("pineapple"));
		one.setPrice(12.0);
		...
		purchaseService.addPizzaToPurchase(one);
		purchaseService.addPizzaToPurchase(two);
		purchaseService.addPizzaToPurchase(three);
		Purchase toReturn = new Purchase();
		toReturn.setPizzas(Arrays.asList(one, two, three));
		when(purchaseRepository.findAllByStateEqualsAndCustomer_Id(any(), any()))
				.thenReturn(Collections.singletonList(new Purchase()));
		when(purchaseRepository.findFirstByStateEquals(any())).thenReturn(new Purchase());
		when(purchaseRepository.save(any())).thenReturn(toReturn);
		...
		assertEquals(toReturn.getAmount(), 22.0);
	}
}
